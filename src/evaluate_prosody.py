...
def main(args, configs, configs_ft):
    ...
    token_length = []
    df_wo_sign = { "duration": [], "pitch": [], "energy": [] }
    df_w_sign  = { "duration": [], "pitch": [], "energy": [] }
    ...
    for batchs in loader:
        for batch in batchs:
            ...
            df_wo_sign["duration"] += d_wo.reshape(-1).tolist()
            df_wo_sign["pitch"] += p_wo.reshape(-1).tolist()
            df_wo_sign["energy"] += e_wo.reshape(-1).tolist()

            # with sign
            d_w = output_w_sign.d_rounded
            p_w = output_w_sign.p_predictions
            e_w = output_w_sign.e_predictions
            df_w_sign["duration"] += d_w.reshape(-1).tolist()
            df_w_sign["pitch"] += p_w.reshape(-1).tolist()
            df_w_sign["energy"] += e_w.reshape(-1).tolist()
