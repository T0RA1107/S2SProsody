...
from .ctrgcn_base_p76 import Model as PoseBackbone
...
class PoseBackboneWrapper(nn.Module):
    def __init__(self):
        ...
        pose_weights = torch.load(current_dir + '/ckpt/ctr_76_e1/runs-88-100144.pt')
        # output [B C T V]
...
class PartedPoseBackbone(nn.Module):
    def __init__(self):
        ...
        self.pose_model = PoseBackbone(
                num_class=2000, num_point=76, num_person=1,
                graph='libs.models.visual_backbone.graph.mmpose_p76.Graph',
                graph_args={'labeling_mode': 'spatial'}, drop_out=0)
        pose_weights = torch.load(current_dir + '/ckpt/ctr_76_e1/runs-88-100144.pt')
        # output [B C T V]
        self.pose_model.load_state_dict(pose_weights, strict=False)
...
class OPPartedPoseBackbone(nn.Module):
    def __init__(self):
        ...
        self.pose_model = PoseBackbone(
                ...
                graph_args={'labeling_mode': 'spatial'}, drop_out=0)
        pose_weights = torch.load(current_dir + '/ckpt/ctr_op78_mix_HF05_F64_e1/runs-82-93316.pt')
        # output [B C T V]
        self.pose_model.load_state_dict(pose_weights, strict=False)
        self.feat_dim = self.pose_model.fc.in_features * 4
