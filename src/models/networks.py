...
class TextConditionalDiscriminator(nn.Module):
    """Defines a Text conditional discriminator"""

    def __init__(self, input_nc, dim_embedding, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d):
        """Construct a Text conditional discriminator
        ...
        Parameters:
            ...
            norm_layer      -- normalization layer
        """
        super(TextConditionalDiscriminator, self).__init__()
        if type(norm_layer) == functools.partial:  # no need to use bias as BatchNorm2d has affine parameters
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
        ...
        sequence += [nn.Conv2d(ndf * nf_mult, dim_embedding, kernel_size=kw, stride=1, padding=padw)]  # output 1 channel prediction map
        self.audio_project = nn.Sequential(*sequence)

        self.crs_attn = FFTBlockCrossAttention(
            dim_embedding,
            2,
            dim_embedding,
            dim_embedding,
        ...
        self.pixel_classifier = nn.Conv2d(dim_embedding, 1, kernel_size=1, stride=1)

    def forward(self, input, text_embedding):
        """Standard forward."""
        audio_embedding = self.audio_project(input)
        B, C, T, M = audio_embedding.shape
        assert C == self.dim_embedding
        audio_embedding = audio_embedding.permute(0, 3, 2, 1).reshape(-1, T, C)
        text_embedding = text_embedding.repeat(M, 1, 1)
        assert audio_embedding.shape[0] == text_embedding.shape[0]
        output = self.crs_attn(audio_embedding, text_embedding)
        output = self.pixel_classifier(output)
        if input.device == torch.device("cuda:0"):
            print("input :", input.shape)
            print("a_emb :", audio_embedding.shape)
