"""Tests for libs/util/tool.py."""
import sys
import os
import importlib.util

_spec = importlib.util.spec_from_file_location(
    "tool",
    os.path.join(os.path.dirname(__file__), "../src/libs/util/tool.py"),
)
tool = importlib.util.module_from_spec(_spec)  # type: ignore[arg-type]
_spec.loader.exec_module(tool)  # type: ignore[union-attr]

import torch
import pytest


def test_get_mask_from_lengths_basic():
    lengths = torch.tensor([3, 5, 2])
    mask = tool.get_mask_from_lengths(lengths)
    assert mask.shape == (3, 5)
    assert mask[0, 3].item() is True
    assert mask[0, 2].item() is False
    assert mask[2, 2].item() is True


def test_get_mask_from_lengths_explicit_max():
    lengths = torch.tensor([2, 4])
    mask = tool.get_mask_from_lengths(lengths, max_len=6)
    assert mask.shape == (2, 6)
    assert mask[0, 2].item() is True
    assert mask[1, 3].item() is False


def test_init_random_seeds_runs():
    tool.init_random_seeds(random_seed=42, rank=0)
    v1 = torch.rand(1).item()
    tool.init_random_seeds(random_seed=42, rank=0)
    v2 = torch.rand(1).item()
    assert v1 == pytest.approx(v2)
